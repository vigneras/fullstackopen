    ...
    if (found) {
      personService.update(found.id, personObject)
        .then(newPerson => {
          ...
          console.log(ex)
          setMessage({ msg: `Server-side error when updating ${personObject.name}`, className: 'error', ex })
      ...
      personService.create(personObject)
        .then(newPerson => {
          ...
          console.log(ex)
          setMessage({ msg: `Server-side error when creating ${personObject.name}`, className: 'error', ex })
          setTimeout(() => setMessage({ msg: null, className: 'info' }), 5000)
        })
    }
  }
  ...
  const deletePerson = (person) => {
    ...
    personService.deleteItem(person.id)
      .then(() => {
        ...
        setMessage({ msg: `Server-side error when deleting ${person.name}`, className: 'error', ex })
        setTimeout(() => setMessage({ msg: null, className: 'info' }), 5000)
      })
  }
